...
const HistoryModal: React.FC<HistoryModalProps> = ({ history, onClose }) => {
    return (
        <Modal title="Histórico de Projetos" onClose={onClose}>
            <div className="mt-4 space-y-3 max-h-96 overflow-y-auto custom-scrollbar pr-2">
                {history.length === 0 ? (
                    <p className="text-gray-400 text-center">Nenhum projeto no seu histórico ainda.</p>
                ) : (
                    history.map(item => (
                        <div key={item.id} className="bg-gray-700 p-3 rounded-lg flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-sm text-gray-400">{item.files} notas criadas</p>
                            </div>
